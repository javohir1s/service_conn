...
func Load() Config {
	...
	config.ServiceName = cast.ToString(getOrReturnDefaultValue("SERVICE_NAME", "catalog_service_go"))
	...
	config.ContentServiceHost = cast.ToString(getOrReturnDefaultValue("ORDER_SERVICE_HOST", "localhost"))
	config.ContentGRPCPort = cast.ToString(getOrReturnDefaultValue("ORDER_SERVICE_PORT", ":8082"))
